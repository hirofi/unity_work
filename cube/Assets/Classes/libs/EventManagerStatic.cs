...
public class EventManagerStatic : MonoBehaviour
{
	public bool LimitQueueProcesing = false;
	public float QueueProcessTime = 0.0f;
	private static EventManagerStatic s_Instance = null;
	private Queue m_eventQueue = new Queue();
	
	public delegate void EventDelegate<T> (T e) where T : GameEventStatic;
	private delegate void EventDelegate (GameEventStatic e);
	...
	public static EventManagerStatic Instance {
		get {
			if (s_Instance == null) {
				s_Instance = GameObject.FindObjectOfType (typeof(EventManagerStatic)) as EventManagerStatic;
			}
			return s_Instance;
		}
	}
	...
	public void AddListener<T> (EventDelegate<T> del) where T : GameEventStatic {
		AddDelegate<T>(del);
	}
	
	public void AddListenerOnce<T> (EventDelegate<T> del) where T : GameEventStatic {
	...
	public void RemoveListener<T> (EventDelegate<T> del) where T : GameEventStatic {
	...
	public bool HasListener<T> (EventDelegate<T> del) where T : GameEventStatic {
